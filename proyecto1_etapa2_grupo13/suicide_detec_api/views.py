    ...
    def post(self, request):
        ...
        for entry in request.data:
            try:
                ...
                predictions.append({'text': entry['text'], 'class': prediction})
...
class Train(views.APIView):
    def post(self, request):
        # Create dataframe 
        df = pandas.DataFrame(columns=['text', 'class'])

        try:
            for entry in request.data:
                texto = entry['text']
                texto = process_text(texto)
                clasification = entry['class']
                if clasification == 'suicide':
                    clasification = 1
                elif clasification == "non-suicide": 
                    clasification = 0
                else:
